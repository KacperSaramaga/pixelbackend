...
app.use(cors({
  origin: "https://dapper-croquembouche-947cbd.netlify.app",
  methods: ["GET", "POST"]
}));
...
io.on('connection', (socket) => {
    ...
    // ✅ Define limited retro colors
    const colors = ["#FF0000", "#00FF00", "#0000FF", "#FFFF00", "#FF00FF", "#00FFFF", "#FFFFFF", "#FFA500"];
    const randomColor = colors[Math.floor(Math.random() * colors.length)];

    // ✅ Ensure users spawn inside the grid
    users[socket.id] = {
        id: socket.id,
        x: Math.floor((Math.random() * 76) + 2) * 5,  // Ensures multiples of 5, avoids edges
        y: Math.floor((Math.random() * 76) + 2) * 5,  
        color: randomColor
    };
